...
//
// CALIBRATION. The card is fitted to jobs Noah has priced by hand:
//
//   Barrie,  ~45km, 15 players, 3 each  ->  $175   (quoted and agreed)
//   Elmvale, ~15km, 12 players, 3 each  ->  $140   (confirmed)
//
//   base 70 + (15 x 6) + ((45 - 20) x 0.60) = 70 + 90 + 15 = 175
//
// Two points, four levers, so it is still underdetermined -- both of these
// sit close to home with small rosters, which is exactly where the card is
// least likely to be wrong. A 40-player tournament an hour away is still
// extrapolation. scripts/check-pricing.mjs asserts both, so an edit that
// quietly reprices the business fails loudly instead.
