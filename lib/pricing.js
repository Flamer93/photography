...
//
// The split between the session fee and the per-player rate is not fixed by
// those two anchors -- both are 15-player jobs, so any pair summing to $135
// fits them and they only diverge on roster size. It was weighted towards
// the per-player rate deliberately: at the earlier split a 30-player team
// cost only $75 more than a 15-player one, and thirty players really is
// closer to twice the work, on the ice and at the desk. The booking minimum
// is what protects the small end now, not the session fee.
//
