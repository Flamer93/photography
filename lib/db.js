...
// A shoot quote a buyer generated on /quote. Stored so Noah sees the request
// and the number they were shown -- a quote he never saw is a quote he cannot
// honour. The price is recorded as given, not recomputed later, because the
// rate card can change after the fact.
...
export async function listQuoteRequests(max = 200) {
  const snap = await getDocs(
    query(quotesCol(), orderBy("createdAt", "desc"), limit(max))
  );
  return mapDocs(snap);
}

export async function setQuoteHandled(quoteId, handled) {
  return updateDoc(doc(db, "quotes", quoteId), { handled });
}
