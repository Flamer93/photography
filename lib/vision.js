...
export async function detectPlayers(photo) {
  ...
  // App Check is set up on first use rather than on page load, so buyers
  // never pull reCAPTCHA in. Awaiting it means the first photo of a run does
  // not fail while the rest succeed; every later call gets the cached promise.
...
export function describeAiError(err) {
  ...
  if (/app check/i.test(raw)) {
    // Two very different situations produce this, and the advice for one is
    // useless for the other: either no site key is configured at all, or one
    // is and something about the registration does not line up.
    return appCheckEnabled
      ? "App Check rejected the request. In the AI project, check that the " +
          "web app is registered under App Check with the reCAPTCHA " +
          "Enterprise provider and this exact site key, and that the key's " +
          "allowed domains include the site you are on."
      : "The AI project enforces App Check but no reCAPTCHA site key is " +
          "configured here, so no token is sent. Set " +
          "NEXT_PUBLIC_AI_RECAPTCHA_SITE_KEY, or unenforce App Check for " +
          "Firebase AI Logic in that project.";
  }
  ...
  if (/prepayment credits|credits are depleted|billing/i.test(raw)) {
    const where = usingSeparateAiProject
      ? `the AI project (${process.env.NEXT_PUBLIC_AI_FIREBASE_PROJECT_ID})`
      : "this project";
    return (
      `Gemini has no credit left on ${where}, so it is refusing the requests. ` +
      "Top it up at https://ai.studio/projects — make sure the project picker " +
      "there is on that one, not another. Nothing in the site needs changing."
    );
  }
